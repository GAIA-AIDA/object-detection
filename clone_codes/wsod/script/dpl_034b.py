...
saved_state_dict = torch.load(os.path.join('../snapshots', 'train_022', 'ckpt_5000'), map_location=torch.device('cpu'))
...
for i, (imgid, imgpath) in enumerate(kf_id_to_img_path.items()):
    ...
    if len(batch) == batch_size or (i + 1) == num_imgs:
        ...
        with torch.no_grad():
            ...
            outputs = resnet(inp)
            outputs = torch.cat([outputs, torch.ones(batch.shape[0], 1)#.cuda(0)
                                ], dim=1)
    ...
    if (i+1) % 100 == 0:
        log(f'Processed {i + 1} out of {num_imgs} images.')
...
save_obj(det_results, f'../../results/det_results_ws_kf_{exp_name}')
